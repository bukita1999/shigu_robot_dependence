...
{
    //初始化节点
    ros::init(argc, argv, "gps2base");
    //声明节点句柄
    ros::NodeHandle nh;
    //发布话题
    enu_pub = nh.advertise<nav_msgs::Odometry>("/gps/odom_base", 10);
    //订阅话题
    ros::Subscriber fix_sub = nh.subscribe("/gps/odom", 1, callback);
    //订阅话题
