...
{
    std::ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/gps_odom_valid_data.txt", ios::app);
    if (!filename)
    {
...
{
    ros::init(argc, argv, "gps_odom_valid_data");
    ros::NodeHandle nh;
    //每次都复写文件
    std::ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/gps_odom_valid_data.txt", ios::trunc);
    filename << "time"
             << "  ";
    filename << "pos_x"
             << "  "
    ...
    filename << "angular_x"
             ...
             << "angular_z" << endl;
    filename.close();
    ros::Subscriber data = nh.subscribe("/gps/odom_valid", 10, callback); //订阅/gps/odom话题
    ros::spin();
    return 0;
}