...
{
    std::ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/gps_data.txt", ios::app);
    if (!filename)
    {
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    //每次都复写文件
    std::ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/gps_data.txt", ios::trunc);
    filename << "time"
             << "  ";
    filename << "latitude"
             << "  "
             ...
             << "altitude" << endl;
    filename.close();
    ros::Subscriber data = nh.subscribe("/navsat/fix", 10, callback); //订阅/navsat/fix话题
    ros::spin();
