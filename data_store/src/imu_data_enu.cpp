...
{
    ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/imu_data_enu.txt", ios::app);
    if (!filename)
    {
...
{
    ros::init(argc, argv, "imu_data_enu");
    ros::NodeHandle nh;
    //每次都复写文件
    ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/imu_data_enu.txt", ios::trunc);
    filename << "time"
             << "  ";
    ...
    filename.close();
    ros::Subscriber data = nh.subscribe("/imu/data_enu", 10, callback); //订阅/imu/data话题
    ros::spin();
