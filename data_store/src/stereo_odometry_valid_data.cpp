...
{
    std::ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/stereo_odometry_valid_data.txt", ios::app);
    if (!filename)
    {
...
{
    ros::init(argc, argv, "stereo_odometry_valid_data");
    ros::NodeHandle nh;
    //每次都复写文件
    std::ofstream filename("/home/sjtu/zjd/ShiGu/src/data_store/data/stereo_odometry_valid_data.txt", ios::trunc);
    filename << "time"
             << "  ";
    filename << "pos_x"
             << "  "
    ...
    filename << "angular_x"
             ...
             << "angular_z" << endl;
    filename.close();
    ros::Subscriber data = nh.subscribe("/stereo_odometry_valid", 10, callback); //订阅/stereo_odometry话题
    ros::spin();
    return 0;
}