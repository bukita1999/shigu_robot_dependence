...
int main(int argc, char **argv)
{
    ...
    ros::Publisher GPS_pub = nh.advertise<sensor_msgs::NavSatFix>("/navsat/fixed", 10);
    try
    {
        //串口设置
            ...
            while (i < strRece.length())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (lat == 0 || lon == 0)
                        {
                            GPS_data.status.status = sensor_msgs::NavSatStatus::STATUS_NO_FIX; //未获取到数据
                        }
                        else
                        {
                            GPS_data.status.status = sensor_msgs::NavSatStatus::STATUS_FIX; //获取到数据
                        }
